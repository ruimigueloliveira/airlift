...
public class MainRepos {
	...
	public static void main(String[] args) {
		...
		try
		{ reposStub = (GeneralReposInt) UnicastRemoteObject.exportObject (repos, portNumb);
		}
		...
		try
		{ while (!serviceEnd)
			synchronized (Class.forName ("serverSide.main.MainRepos"))
			{ try
			{ (Class.forName ("serverSide.main.MainRepos")).wait ();
			}
		...
		{ GenericIO.writelnString ("The data type MainRepos was not found (blocking)!");
		e.printStackTrace ();
		System.exit (1);
		}
	...
	public static void shutdown ()
	{
		...
		try
		{ synchronized (Class.forName ("serverSide.main.MainRepos"))
			{ (Class.forName ("serverSide.main.MainRepos")).notify ();
			}
		}
		catch (ClassNotFoundException e)
		{ GenericIO.writelnString ("The data type MainRepos was not found (waking up)!");
		e.printStackTrace ();
		System.exit (1);
		}
	}
